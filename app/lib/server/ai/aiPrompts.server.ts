...

{
  ...
  "lines": [
    { "line": 1, "code": "import type { ... } from \\"...\\";" },
    { "line": 2, "code": "" },
    { "line": 27, "code": "export const loader = async (...) => {", "note": "定义 root loader, 进入页面前先准备全局数据" },
    { "line": 28, "code": "  const theme = await getTheme(request);", "note": "读取主题 cookie, 决定 html/body 初始主题" }
  ],
  "blockNotes": [
    {
...
- **必须把输入源码每一行都吐出来**, 从 line 1 一直到最后一行, **一行都不能跳, 不能合并, 不能省略**。
  - 空行也要出现, 写成 \`{ "line": N, "code": "" }\`。
  - 不要给"省略相似行"或"// ...省略" — 一行不少地完整 echo。
- code 字段 = 该行**原始源码**, 不做任何加工 (引号 / 空格 / 注释一字不改; 转义遵循 JSON 规范)。
- note 字段 = 可选行内旁批:
  - 给关键的 5-15 行加 note (按文件长度可适当增减), 其它行只写 code 不写 note。
  - 一句话, **不超过 30 字**, 像在这行后面写的 // 注释。
  - 不重复 / 不啰嗦; 不要 "这是一个 useEffect" 这种废话。
  - 例: line 27 \`export const loader = ...\` → note: "定义 root loader, 准备全局数据"
- **行号锚定铁律 (常见错误)**:
  - line 字段 = 这行在输入里的真实行号, 抄输入里 \`<行号> | <代码>\` 最左边那个数字。
  - 输出顺序按 line 升序, 不要乱序。
  - 由于你自己输出 code 字段, 行号和内容是同步生成的 — 这是这个格式的关键优势:
    **请确保 lines[N].code 完整匹配输入里 line == lines[N].line 的那一行代码**, 不要张冠李戴。

## blockNotes (段讲解块)

- 2 ~ 5 段, 选关键代码段做深入讲解 (跨多行的因果点, 而不是单行琐事)。
- 按 startLine 升序; 段之间不要互相覆盖大段重叠 (允许小范围嵌套但要有意义)。
- **startLine / endLine 必须是 lines 数组里真实出现过的行号**, 直接抄, 不要自己估。
- level 选取:
  - **basic**: 普通说明, 帮助新人理解角色。
...
- 输出 lines 数组, **每一行原样 echo 一遍** (从 line 1 到末行, 一行不少), 对其中关键的 5-15 行额外挂 note 字段做旁批 (像 // 注释)。
- 输出 blockNotes 数组, 给关键代码段插入多层级讲解 (角色 / 因果 / 改坏陷阱 / 修改建议), 帮学习者建立因果地图。

**为什么要 echo 全部代码**: 由于你自己输出代码和行号, 行号和内容天然同步, 不会出现"批注挂错行"的问题。**不要省略任何一行, 不要合并相似行**。
...
你会拿到一个源码文件的完整代码、一道题、用户的作答、以及正确答案。请**结合用户的作答**, 输出「源码精读讲义」:
- lines 数组: 每行原样 echo + 关键行挂 note (一句话, // 风格旁批)。
- blockNotes 数组: 给关键代码段插入多层级讲解, 用户答错时至少 1 条 level=risk + 配对 suggestion; 答对时聚焦"相邻陷阱 / 边界条件 / AI 易改坏处"。
