...
export function buildExplanationPrompt(
  ...
  promptParts.push(
    "",
    "只输出 system 要求的 JSON。把讲解锚定到与这道题真正相关的行, 不要泛讲、不要重出完整代码方案。",
  );
...
/** 两个"行锚定讲解"特性(导读 + 结合答案讲解)共用的 JSON 输出契约。 */
const ANNOTATION_OUTPUT_CONTRACT = `## 输出格式 (硬性)

只输出一个 JSON 对象, 不要任何额外文字, 不要 markdown 代码围栏 (不要出现 \`\`\`):
{
...
你会拿到一个源码文件的完整代码。你的任务是做"读前导读": 帮学习者快速建立这个文件的因果地图——角色边界、请求/事件流向、关键行各自在干嘛、AI 来改这里最容易改坏什么。
...
你会拿到一个源码文件的完整代码、一道题、用户的作答、以及正确答案。请**结合用户的作答**, 把讲解锚定到真正相关的代码行上:
- 用户答错: 指出他的理解偏在哪, 用代码的哪几行能纠正这个误解。
...
export function buildCodeOrientationPrompt(input: {
  ...
  promptParts.push(
    "",
    "只输出 system 要求的 JSON。只讲代码本身, 不要剧透或暗示任何题目答案。",
  );
